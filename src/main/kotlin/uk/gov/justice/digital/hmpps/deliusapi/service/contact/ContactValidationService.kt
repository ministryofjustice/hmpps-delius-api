...
class ContactValidationService(
  ...
  fun validateReplaceContactOffenderId(offenderId: Long, contact: Contact) {
    if(offenderId != contact.offender.id){
      throw BadRequestException("Offender ID does not match the offender ID on the contact")
    }
  }
